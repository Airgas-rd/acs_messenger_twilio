...
async def main():
    try:
        ...
        await initialize() # set up db connection and clients

        queue = asyncio.Queue(maxsize=max_queue_size)

        # Workers to process the queue concurrenty
        tasks = []
...
async def initialize():
    try:
        ...
        conn = psycopg2.connect(**db_params, cursor_factory = DictCursor, async_ = True)
...
async def send_sms(record: dict):
    try:
...
async def send_email(record: dict):
    try:
        if email_override:
